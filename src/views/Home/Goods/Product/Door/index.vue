...
export default {
    ...
    methods: {
         ...
         */
        async getDoorByPageNum(pageNum = 1, pageSize = this.paginationOptions.pageSize) {
            try {
                const {code, msg, data} = await this.$axios.get(productApis.getDoorByPageNum, {
                    params: {
                        pageNum,
                        pageSize
                    }
        ...
        async changeStatus(type, row){
            try {
                const { code, msg } = await this.$axios.post(productApis.changeStatusDoor, {
